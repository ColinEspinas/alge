...
import { Container, utils, Renderer, settings, SCALE_MODES, Texture, Sprite as Sprite$1, RenderTexture, BaseRenderTexture, Point, BaseTexture, Rectangle } from 'pixi.js';
...
class RigidBody extends Component {
    ...
    Create() {
        ...
        //==== Collision Events : ====//
        Events.on(this.GetManager(PhysicsManager).physicsEngine, 'collisionStart', event => {
        ...
        Events.on(this.GetManager(PhysicsManager).physicsEngine, 'collisionEnd', event => {
            var pairs = event.pairs;
            for (var i = 0, len = pairs.length; i < len; ++i) {
                const pair = pairs[i];
        ...
        Events.on(this.GetManager(PhysicsManager).physicsEngine, 'collisionActive', event => {
            var pairs = event.pairs;
            for (var i = 0, len = pairs.length; i < len; ++i) {
                const pair = pairs[i];
...
class Camera {
    ...
    MoveTo(position, options) {
        ...
        if (position.Distance(this.position) > tolerance) {
            const point = new Point(options.function(options.time * this.deltaTime * 100 || 1, this.viewport.center.x, position.x, options.duration * this.deltaTime * 100) || position.x, options.function(options.time * this.deltaTime * 100 || 1, this.viewport.center.y, position.y, options.duration * this.deltaTime * 100) || position.y);
    ...
    MoveToHorizontal(position, options) {
        const tolerance = options.tolerance || 0.5;
        if (position.Distance(this.position) > tolerance) {
            const point = new Point(options.function(options.time * this.deltaTime * 100 || 1, this.viewport.center.x, position.x, options.duration * this.deltaTime * 100) || position.x, this.viewport.center.y);
            this.viewport.moveCenter(point);
        }
    }
    MoveToVertical(position, options) {
        const tolerance = options.tolerance || 0.5;
        if (position.Distance(this.position) > tolerance) {
            const point = new Point(this.viewport.center.x, options.function(options.time * this.deltaTime * 100 || 1, this.viewport.center.y, position.y, options.duration * this.deltaTime * 100) || position.y);
            this.viewport.moveCenter(point);
        }
    }
    AddTrauma(amount) {
