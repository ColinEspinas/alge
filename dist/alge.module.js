...
import { Container, Graphics, utils, Renderer, settings, SCALE_MODES, Texture, Sprite as Sprite$1, Text as Text$1, RenderTexture, BaseRenderTexture, Point, BaseTexture, Rectangle } from 'pixi.js';
...
class Camera$1 {
    ...
    set position(position) { this.viewport.center = new Point(position.x, position.y); }
    WorldToCamera(position) {
    ...
    MoveTo(position, options) {
        ...
        if (position.Distance(this.position) > tolerance) {
            const point = new Point(Math.floor(Ease.lerp(this.viewport.center.x, position.x, options.duration * (this.deltaTime * 100))), Math.floor(Ease.lerp(this.viewport.center.y, position.y, options.duration * (this.deltaTime * 100))));
    ...
    MoveToHorizontal(position, options) {
        const tolerance = options.tolerance || 0.5;
        if (position.Distance(this.position) > tolerance) {
            const point = new Point(Math.floor(Ease.lerp(this.viewport.center.x, position.x, options.duration * (this.deltaTime * 100))), this.viewport.center.y);
            this.viewport.moveCenter(point);
        }
    }
    MoveToVertical(position, options) {
        const tolerance = options.tolerance || 0.5;
        if (position.Distance(this.position) > tolerance) {
            const point = new Point(this.viewport.center.x, Math.floor(Ease.lerp(this.viewport.center.y, position.y, options.duration * (this.deltaTime * 100))));
            this.viewport.moveCenter(point);
        }
    }
    AddTrauma(amount) {
...
export { Angle, BaseScene, BaseSceneManager, Camera, Camera$1 as CameraUtil, Component, Cursor, DebugCollider, Ease, engine as Engine, Entity, InputManager, Key, Mouse, Noise, PhysicsManager, RenderManager, RigidBody, Scene, SceneManager, Sprite, SpriteMode, Text, Tilemap, Tileset, TimeManager, Transform, Vec, Viewport };
